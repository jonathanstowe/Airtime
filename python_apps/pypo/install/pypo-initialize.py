...
PATH_LIQUIDSOAP_BIN = '/usr/local/lib/airtime/pypo/bin/liquidsoap_bin'
...
try:
    ...
    symlink_path = "/usr/local/bin/airtime-liquidsoap"
    ...
    if "airtime_service_start" in os.environ and os.environ["airtime_service_start"] == "t":
        ...
        subprocess.call("/usr/local/etc/rc.d/airtime_liquidsoap start > /dev/null 2>&1", shell=True)
        subprocess.call("/usr/local/etc/rc.d/airtime_playout start > /dev/null 2>&1", shell=True)
